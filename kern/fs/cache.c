...
static cache_t* cache_add(int sec, void *buf)
{
...
static int cache_reduce_list()
{
    ...
    while(true) 
    {
        tmp = le;
        //reduce 1/10 lists
        if(le == CACHE_UNFSYN_LIST_P || index == cache_manager.limit / 10)
            break;
        le = list_next(le);
        t = list2cache(tmp);
        assert(t->flags == BIT_INVALID || t->flags == BIT_WRITING);
